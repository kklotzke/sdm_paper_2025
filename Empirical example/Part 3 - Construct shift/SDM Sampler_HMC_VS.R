...
# First group is treated as reference group with ginter = tinter = 0
#
# Konrad Klotzke
...
SamplerSDM_HMC_VS <- function(Y.list, u, dtype = 1, ftype = c("UD", "MD", "BF"), fix.ginter = FALSE, fix.tinter = FALSE, fix.cor = FALSE, epsilon, epsilon.scale, epsilon.sd, L, L.sd, M, enforceLB.tau = TRUE, pr.tau.lower = -2, pr.tau.upper = 2, pr.sig2k.upper = 10, pr.iinter.mu0 = 0, pr.iinter.var0 = 10, a0 = 0.0001, b0 = 0.0001, XG, XG.Gibbs, Burnin.Gibbs, runHMC = TRUE, runGibbs = TRUE, Gibbs.trmethod = "ICDF", Gibbs.tau.start = NULL, Gibbs.Z.start = NULL, pre.est = "mean", pre.iinter = NULL, pre.sig2k = NULL, pre.tau = NULL, pre.Z = NULL, save.Z = FALSE, save.Z.Gibbs = FALSE, save.DistPar = FALSE, ii.print = 100, ii.print.Gibbs = 100, verbose = FALSE) {
  ...
  if (dtype == 1) {
    ...
    if (fix.cor) {
      SSWk.all <- numeric(K)
      for (kk in 1:K) {
        SSWk.all[kk] <- sum((Z.all[, kk] - mean(Z.all[, kk])) ^ 2)
      }
    }
  }
  else if (dtype == 2) {
  ...
  if (runHMC) {
    ...
    for (ii in 2:XG) {
      ...
      # Latent responses and data structures
      for (gg in 1:Ng) {
        ...
        if (save.DistPar && (dtype == 1)) {
          gdata[[gg]]$DistPar.sig2k[[ii]] <- matrix(NA, nrow = K, ncol = 6) # shift, scale, shape, rate, a, b
        }
      }

      if (!fix.cor) {
        free.cor <- TRUE # Estimate covariances per group
        for (gg in 1:Ng) {
          L.mean <- gdata[[gg]]$L
          ...
          iinter <- gdata[[gg]]$Samples.iinter[ii - 1,]
          sig2k <- gdata[[gg]]$Samples.sig2k[ii - 1,]
          free.ginter <- !fix.ginter && (gg != 1)
          free.tinter <- !fix.tinter && (gg != 1)

          # Current position parameter(s)
          tau <- gdata[[gg]]$Samples.tau[ii - 1,]

          # Mean structure 
          B.sum <- gdata[[gg]]$B.sum

          # Covariance structure
          Sigma <- gdata[[gg]]$Sigma
          ...
          G <- getGrad(x = tau.star, Z = gdata[[gg]]$Z, sig2k = sig2k, ginter = ginter, tinter = tinter, iinter = iinter, u = u, pr.tau.lower = pr.tau.lower, pr.tau.upper = pr.tau.upper, pr.sig2k.upper = pr.sig2k.upper, pr.iinter.mu0 = pr.iinter.mu0, pr.iinter.var0 = pr.iinter.var0, a0 = a0, b0 = b0, sig2k.shift = gdata[[gg]]$sig2k.shift, which.f1 = which.f1, free.cor = free.cor, free.ginter = free.ginter, free.tinter = free.tinter)
          ...
          # Metropolis step
          U.min1 <- -lpost(x = tau, Z = gdata[[gg]]$Z, sig2k = sig2k, ginter = ginter, tinter = tinter, iinter = iinter, u = u, pr.tau.lower = pr.tau.lower, pr.tau.upper = pr.tau.upper, pr.sig2k.upper = pr.sig2k.upper, pr.iinter.mu0 = pr.iinter.mu0, pr.iinter.var0 = pr.iinter.var0, a0 = a0, b0 = b0, sig2k.shift = gdata[[gg]]$sig2k.shift, which.f1 = which.f1, free.cor = free.cor, free.ginter = free.ginter, free.tinter = free.tinter)
          K.min1 <- sum((p ^ 2) / (2 * diag(gdata[[gg]]$M))) # (t(p) %*% solve(M) %*% p)[1, 1] / 2
          H.min1 <- U.min1 + K.min1
          U.cand <- -lpost(x = tau.star, Z = gdata[[gg]]$Z, sig2k = sig2k, ginter = ginter, tinter = tinter, iinter = iinter, u = u, pr.tau.lower = pr.tau.lower, pr.tau.upper = pr.tau.upper, pr.sig2k.upper = pr.sig2k.upper, pr.iinter.mu0 = pr.iinter.mu0, pr.iinter.var0 = pr.iinter.var0, a0 = a0, b0 = b0, sig2k.shift = gdata[[gg]]$sig2k.shift, which.f1 = which.f1, free.cor = free.cor, free.ginter = free.ginter, free.tinter = free.tinter)
        ...
        gg <- NULL
        free.cor <- FALSE # Fix covariances across groups
        L.mean <- gdata[[1]]$L # HMC tuning parameters are equal across groups
        epsilon.mean <- gdata[[1]]$epsilon

        sig2k <- gdata[[1]]$Samples.sig2k[ii - 1,] # Equal across groups
        iinter <- gdata[[1]]$Samples.iinter[ii - 1,] # Equal across groups
        free.ginter <- !fix.ginter
        free.tinter <- !fix.tinter

        # Current position parameter(s)
        tau <- gdata[[1]]$Samples.tau[ii - 1,] # Equal across groups

        # Mean structure 
        ginter.list <- list() # Per group
        tinter.list <- list() # Per group
        for (gg in 1:Ng) {
          ginter.list[[gg]] <- gdata[[gg]]$Samples.ginter[ii - 1,]
          tinter.list[[gg]] <- gdata[[gg]]$Samples.tinter[ii - 1,]
        }

        # Latent responses
        Z.list <- list() # Per group
        for (gg in 1:Ng) {
          Z.list[[gg]] <- gdata[[gg]]$Z
        }
        ...
        # Sample momentum parameter(s)
        p <- MASS::mvrnorm(n = 1, mu = rep(0, Nt), Sigma = gdata[[1]]$M)

        # Vary number of steps for better parameter space exploration / to prevent getting stuck in a loop
        L <- round(runif(1, L.mean - gdata[[1]]$L.sd, L.mean + gdata[[1]]$L.sd))

        # Vary step size for better parameter space exploration / to prevent getting stuck in a loop
        epsilon <- runif(1, epsilon.mean - gdata[[1]]$epsilon.sd, epsilon.mean + gdata[[1]]$epsilon.sd)
        ...
        # Half step for momentum parameter(s)
        G <- getGrad(x = tau.star, Z = Z.list, sig2k = sig2k, ginter = ginter.list, tinter = tinter.list, iinter = iinter, u = u, pr.tau.lower = pr.tau.lower, pr.tau.upper = pr.tau.upper, pr.sig2k.upper = pr.sig2k.upper, pr.iinter.mu0 = pr.iinter.mu0, pr.iinter.var0 = pr.iinter.var0, a0 = a0, b0 = b0, sig2k.shift = gdata[[1]]$sig2k.shift, which.f1 = which.f1, free.cor = free.cor, free.ginter = free.ginter, free.tinter = free.tinter)
        p.star <- p.star - gdata[[1]]$epsilon.scale * epsilon * G / 2
        ...
        while (ll <= L) {
          if ((ll == 1) | ((ll > 1) & !all(G == 0))) {
            ...
            # Full step for position parameter(s)
            if (!enforceLB.tau) {
              tau.star <- tau.star + gdata[[1]]$epsilon.scale * epsilon * p.star / diag(gdata[[1]]$M)
            }
            else {
              for (tt in 1:Nt) {
                # Full step for position parameter(s) 
                tau.star[tt] <- tau.star[tt] + gdata[[1]]$epsilon.scale[tt] * epsilon * p.star[tt] / (diag(gdata[[1]]$M)[tt])
                lb <- getLB(x = tau.star, sig2k = sig2k, u = u, which.tt = tt)
                while (tau.star[tt] < lb) {
                  # Enforce lower bound
                  tau.star[tt] <- lb + (lb - tau.star[tt])
            ...
            if (ll != L) {
              G <- getGrad(x = tau.star, Z = Z.list, sig2k = sig2k, ginter = ginter.list, tinter = tinter.list, iinter = iinter, u = u, pr.tau.lower = pr.tau.lower, pr.tau.upper = pr.tau.upper, pr.sig2k.upper = pr.sig2k.upper, pr.iinter.mu0 = pr.iinter.mu0, pr.iinter.var0 = pr.iinter.var0, a0 = a0, b0 = b0, sig2k.shift = gdata[[1]]$sig2k.shift, which.f1 = which.f1, free.cor = free.cor, free.ginter = free.ginter, free.tinter = free.tinter)
              p.star <- p.star - gdata[[1]]$epsilon.scale * epsilon * G
            }
            if (any(!is.finite(p.star)))
              browser()
          }
        ...
        # Metropolis step
        U.min1 <- -lpost(x = tau, Z = Z.list, sig2k = sig2k, ginter = ginter.list, tinter = tinter.list, iinter = iinter, u = u, pr.tau.lower = pr.tau.lower, pr.tau.upper = pr.tau.upper, pr.sig2k.upper = pr.sig2k.upper, pr.iinter.mu0 = pr.iinter.mu0, pr.iinter.var0 = pr.iinter.var0, a0 = a0, b0 = b0, sig2k.shift = gdata[[1]]$sig2k.shift, which.f1 = which.f1, free.cor = free.cor, free.ginter = free.ginter, free.tinter = free.tinter)
        K.min1 <- sum((p ^ 2) / (2 * diag(gdata[[1]]$M))) # (t(p) %*% solve(M) %*% p)[1, 1] / 2
        H.min1 <- U.min1 + K.min1
        U.cand <- -lpost(x = tau.star, Z = Z.list, sig2k = sig2k, ginter = ginter.list, tinter = tinter.list, iinter = iinter, u = u, pr.tau.lower = pr.tau.lower, pr.tau.upper = pr.tau.upper, pr.sig2k.upper = pr.sig2k.upper, pr.iinter.mu0 = pr.iinter.mu0, pr.iinter.var0 = pr.iinter.var0, a0 = a0, b0 = b0, sig2k.shift = gdata[[1]]$sig2k.shift, which.f1 = which.f1, free.cor = free.cor, free.ginter = free.ginter, free.tinter = free.tinter)
        K.cand <- sum((p.star ^ 2) / (2 * diag(gdata[[1]]$M))) #  (t(p.star) %*% solve(M) %*% p.star)[1, 1] / 2
        H.cand <- U.cand + K.cand
        ...
        for (kk in 1:K) {
          ...
          if (dtype == 1) {
            ...
            CD.L <- invgamma::pinvgamma(lower, shape = a0 + N.all / 2, rate = b0 + SSWk.all[kk] / 2, lower.tail = TRUE) # Lower bound sig2k = 0
            CD.U <- invgamma::pinvgamma(upper, shape = a0 + N.all / 2, rate = b0 + SSWk.all[kk] / 2, lower.tail = TRUE) # Upper bound sig2k = pr.sig2k.upper
            sig2k[kk] <- invgamma::qinvgamma(runif(1) * (CD.U - CD.L) + CD.L, shape = a0 + N.all / 2, rate = b0 + SSWk.all[kk] / 2) - shift
          }
          else if (dtype == 2) {
            # Compute measurement error variance parameter
            sig2k[kk] <- 1 - shift
      ...
      if (free.tinter) {
        ...
        for (gg in 1:Ng) {
          ...
          if (!is.null(which.f1)) {
            gdata[[gg]]$Samples.tinter[ii, which.f1] <- ginter[gg] 
          }
        }
      }
  ...
  return(list("gdata" = gdata, "fix.ginter" = fix.ginter, "fix.tinter" = fix.tinter, "fix.cor" = fix.cor, "which.f1" = which.f1
...
, pr.tau = "Uniform", tau.shift = NULL, tau.scale = NULL, which.f1 = NULL, free.cor, free.ginter, free.tinter) {
  if (free.cor) {
    K <- ncol(Z)
  }
  else {
    K <- ncol(Z[[1]]) # Balanced groups
  }
  Nt <- length(x)
  if (length(pr.tau.lower) == 1)
    pr.tau.lower <- rep(pr.tau.lower, Nt)
  ...
  if (free.cor) {
    ...
    for (gg in 1:Ng) {
      ...
      if (gg != 1) {
        if (free.ginter) {
          mu.list[[gg]] <- mu.list[[gg]] + ginter[[gg]]
        }
        if (free.tinter) {
          for (tt in 1:Nt) {
            if (is.null(which.f1) || tt != which.f1) {
              mu.list[[gg]] <- mu.list[[gg]] + tinter[[gg]][tt] * u[tt,]
            }
          }
        }
      }
  ...
  if (all(is.finite(Sigma)) && all(eigen(Sigma)$values > 0)) {
    if (free.cor) {
      ...
      llik <- 0
      for (gg in 1:Ng) {
        llik <- llik + sum(Rfast::dmvnorm(Z[[gg]], mu = mu.list[[gg]], sigma = Sigma, logged = TRUE)) # Likelihood data
        if (free.ginter) {
          llik <- llik + sum(dnorm(ginter[[gg]], mean = pr.iinter.mu0, sd = sqrt(pr.iinter.var0), log = TRUE)) # Normal prior group means
        }
        if (free.tinter) {
          if (is.null(which.f1)) {
            llik <- llik + sum(dnorm(tinter[[gg]], mean = pr.iinter.mu0, sd = sqrt(pr.iinter.var0), log = TRUE)) # Normal prior factor means 
          }
          else if (!is.null(which.f1) && Nt > 1) {
            llik <- llik + sum(dnorm(tinter[[gg]][-which.f1], mean = pr.iinter.mu0, sd = sqrt(pr.iinter.var0), log = TRUE)) # Normal prior factor means, exclude f1 mean
          }
        }
        llik <- llik + sum(dnorm(iinter, mean = pr.iinter.mu0, sd = sqrt(pr.iinter.var0), log = TRUE)) # Normal prior item intercepts 
      }
...
getGrad <- function(x, Z, sig2k, ginter, tinter, iinter, u, pr.tau.lower, pr.tau.upper, pr.sig2k.upper = NULL, pr.iinter.mu0, pr.iinter.var0, a0 = NULL, b0 = NULL, sig2k.shift = NULL, which.f1 = NULL, free.cor, free.ginter, free.tinter) {
  G <- as.vector(rootSolve::gradient(f = lpost, x = x, Z = Z, sig2k = sig2k, ginter = ginter, tinter = tinter, iinter = iinter, u = u, pr.tau.lower = pr.tau.lower, pr.tau.upper = pr.tau.upper, pr.sig2k.upper = pr.sig2k.upper, pr.iinter.mu0 = pr.iinter.mu0, pr.iinter.var0 = pr.iinter.var0, a0 = a0, b0 = b0, sig2k.shift = sig2k.shift, neg = TRUE, which.f1 = which.f1, free.cor = free.cor, free.ginter = free.ginter, free.tinter = free.tinter))
  # if (any(!is.finite(G))) {
  #   print(G)
  # }
