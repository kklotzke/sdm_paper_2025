...
for (cc in 1:nChains) {
  cat("*** Chain:", cc, "\n")
  out <- SamplerSDM_HMC_VS(Y = Y, u = u, dtype = dtype, ftype = ftype, fix.ginter = fix.ginter, fix.tinter = fix.tinter, fix.cor = fix.cor, epsilon = epsilon, epsilon.scale = epsilon.scale, epsilon.sd = epsilon.sd, enforceLB.tau = TRUE
                            ...
                            , ii.print = 1000, ii.print.Gibbs = 100, verbose = FALSE)
  for (gg in 1:Ng) {
