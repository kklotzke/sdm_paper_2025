...
SamplerSDM_HMC <- function(Y, u, dtype = 1, ftype = c("UD", "MD", "BF"), epsilon, epsilon.scale, epsilon.sd, L, L.sd, M, enforceLB.tau = TRUE, pr.tau.lower = -2, pr.tau.upper = 2, pr.sig2k.upper = 10, pr.iinter.mu0 = 0, pr.iinter.var0 = 10, a0 = 0.0001, b0 = 0.0001, XG, XG.Gibbs, Burnin.Gibbs, runHMC = TRUE, runGibbs = TRUE, Gibbs.trmethod = "ICDF", Gibbs.tau.start = NULL, Gibbs.Z.start = NULL, pre.est = "mean", pre.iinter = NULL, pre.sig2k = NULL, pre.tau = NULL, pre.Z = NULL, save.Z = FALSE, save.Z.Gibbs = FALSE, save.DistPar = FALSE, ii.print = 100, ii.print.Gibbs = 100, verbose = FALSE) {
  ...
  if (save.Z) {
    Z.vec <- matrix(NA, nrow = XG, ncol = N*K)
  }
  else {
    Z.vec <- NULL
  }
  ...
  if (save.Z.Gibbs) {
    Z.vec <- matrix(NA, nrow = XG.Gibbs, ncol = N*K)
  }
  else {
    Z.vec.Gibbs <- NULL
  }
  ...
  if (runHMC) {
    ...
    if (dtype == 1) {
      ...
      Samples.sig2k[1,] <- numeric(K)
      for (kk in 1:K) {
        shift <- 0
        for (tt in 1:Nt) {
          shift <- shift + Samples.tau[1, tt] * u[tt, kk] ^ 2
        }
        sig2k.shift[kk] <- shift
        Samples.sig2k[1, kk] <- 1 - shift
      }
    }
    if (missing(M)) {
      #M <- diag(Nt)
  ...
  , "u" = u, "epsilon" = epsilon, "epsilon.scale" = epsilon.scale, "epsilon.sd" = "epsilon.sd", "L" = L, "L.sd" = L.sd, "M" = M, "enforceLB.tau" = enforceLB.tau, "dtype" = dtype, "ftype" = ftype, "slimit" = slimit
  ...
  , "Z.vec" = Z.vec, "Z.vec.Gibbs" = Z.vec.Gibbs, "DistPar.sig2k" = DistPar.sig2k, "DistPar.sig2k.Gibbs" = DistPar.sig2k.Gibbs, "DistPar.tau.Gibbs" = DistPar.tau.Gibbs))
}
